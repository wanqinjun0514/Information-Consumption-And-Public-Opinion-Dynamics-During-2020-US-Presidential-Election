...
def user_bias_by_without_url():
    ...
    url_score_file_path = str(rp.MEDIA_BIAS_USERNAME_CSV)
    ...
    for month in months:
        ...
        folder_path = str(rp.THREE_PARTS_OUTPUT / "without_url" / f"output_{month}")
        ...
        for filename in os.listdir(folder_path):
            if filename.endswith("-output.csv"):
                ...
                for index, row in df.iterrows():
                    ...
                    retweet_user_id = row['retweet_origin_user_id']
                    quoted_user_id = row['quoted_origin_user_id']

                    if pd.notna(retweet_user_id):
                        # print(f"{user_id}   1")
                        every_mouth_retweet_num += 1
                        score = url_score.get(retweet_user_id)
                        ...
                        score = url_score.get(quoted_user_id)
        ...
        out_dir = rp.ensure_dir(rp.DIR_02_MEDIA_RATING_ROOT / "without_url-rating")
