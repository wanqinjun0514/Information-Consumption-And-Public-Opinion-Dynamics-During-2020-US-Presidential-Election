...
def get_bias_dict():
    file_path = str(rp.POLITICIAN_URL_BIAS_CSV)
...
def extract_monthly_politician_influence_external_url(value_map):
    months = [
        '2019_12',
    ...
    for month in months:
        # Example usage
        input_folder = str(rp.THREE_PARTS_OUTPUT / "external_url" / f"output_{month}")
        if not os.path.isdir(input_folder):
            print(f"skip directory : {input_folder}")
            continue
        output_folder = str(rp.DIR_02_POLITICIAN_INFLUENCE)
        statistic_results_file = str(rp.DIR_02_POLITICIAN_INFLUENCE / f"simplified_forwarding_stats_{month}.csv")
...
def calculate_monthly_user_bias_from_simplified_forwarding():
    ...
    inf_dir = rp.DIR_02_POLITICIAN_INFLUENCE
    for month in months:
        ...
        result_df = pd.DataFrame(result_data)
        out_dir = rp.ensure_dir(rp.DIR_02_POLITICIAN_RATING_ROOT / "external_url-rating" / "monthly_users_bias")
...
if __name__ == '__main__':
    ...
    extract_monthly_politician_influence_external_url(bias_dict)
